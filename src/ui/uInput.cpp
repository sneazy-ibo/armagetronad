...
{
    // the description is converted at the call: tOutput hands out a rotating
    // buffer, and tString::Len() counts the terminator, so the label needs size()
    if ( label.size() > 0 )
        DisplayText(REAL(x-.02),y,label,selected,alpha,1);
    else
