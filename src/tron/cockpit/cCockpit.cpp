...
static int sg_storedMapState( tString const & path )
{
    ...
    while ( pos < hud_map_states.Len() )
    {
        tString::size_type end = hud_map_states.find( " ", pos );
        tString const token = hud_map_states.substr( pos, ( end == tString::npos ? hud_map_states.Len() : end ) - pos );

        if ( token.Len() > 2 && token[1] == ':' && token.substr( 2 ) == path )
...
static void sg_storeMapState( tString const & path, bool hidden )
{
    ...
    while ( pos < hud_map_states.Len() )
    {
        ...
        bool const mine = token.Len() > 2 && token[1] == ':' && token.substr( 2 ) == path;
        if ( !mine && token.Len() > 0 )
            out << token << " ";
